...
function Models() {
  ...
  return (
    <>
      ...
      <Section className="!pt-2" title="Cost comparison" hint="Drag to compare monthly cost based on volume.">
        <div className="space-y-8">
          
          {/* Slider */}
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-[13px] text-muted-foreground">Monthly volume</span>
              <span className="font-display text-[14px] font-semibold text-foreground tabular-nums">
                {chars.toLocaleString()} <span className="text-[12px] font-normal text-muted-foreground">chars</span>
              </span>
            </div>
          ...
          <div className="flex items-end gap-3.5 justify-center pt-8 pb-3 border-b border-border/40 h-[280px] group/chart">
            {MODELS_DATA.map((m) => {
              const totalCost = (chars / 1000) * m.ratePer1k;
              // Global max cost corresponds to 2M characters volume at max rate of 10.00 (₹20,000)
              const globalMaxCost = (2000000 / 1000) * 10.00;
              // Bars grow taller up to 210px as volume changes
              const heightPct = Math.max(12, (totalCost / globalMaxCost) * 210);
              ...
              return (
                ...
                >
                  ...
                  <div className="absolute bottom-[calc(100%+30px)] left-1/2 -translate-x-1/2 z-20 w-36 scale-95 opacity-0 pointer-events-none rounded-lg border border-border bg-background p-2.5 shadow-lg group-hover/bar:scale-100 group-hover/bar:opacity-100 transition-all duration-150">
                    <div className="font-semibold text-foreground text-[11.5px] truncate">{m.name}</div>
                    <div className="mt-1 flex items-center justify-between text-[11px] text-muted-foreground">
                      <span>Latency:</span>
                      <span className="text-foreground font-medium">{m.latency}</span>
                    </div>
                    <div className="flex items-center justify-between text-[11px] text-muted-foreground">
                      <span>Rate / 1k:</span>
                      <span className="text-foreground font-medium">₹{m.ratePer1k.toFixed(2)}</span>
                    </div>
                  </div>
      ...
      <Section title="Tradeoff matrix" hint="Steerability, performance metrics, and cost tiers compared.">
        <div className="-mx-3 pt-1">
          {/* Header Row */}
          <div className="grid grid-cols-[1.5fr_1.2fr_1fr_0.8fr] gap-6 px-3 pb-3 eyebrow-label border-b border-border/30">
              ...
              return (
                ...
                >
                  ...
                  <div className="space-y-0.5 min-w-0">
                    <span className="text-[14px] font-semibold text-foreground truncate block">{m.name}</span>
                    <span className="text-[11.5px] text-muted-foreground/60 block">
                      {m.isExternal ? "External API integration" : "Rumik native model"}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[12px] text-muted-foreground font-medium">
                      {m.speedScore >= 90 ? "Sub-second" : m.speedScore >= 75 ? "Snappy" : "Buffered"}
                    </span>
                  </div>

                  {/* Cost rate column */}
                  <div className="text-[12.5px] font-mono text-foreground tabular-nums">
                    ₹{m.ratePer1k.toFixed(2)} <span className="text-[11.5px] font-sans text-muted-foreground font-normal">/ 1K chars</span>
                  </div>

                  {/* Price Tier Pill */}
                  <div className="text-right">
                    <span className={`inline-block rounded px-2.5 py-0.5 text-[11px] font-semibold uppercase tracking-wider ${
                      m.ratePer1k <= 0.35 
          ...
          {MODELS_DATA.filter(m => !m.isExternal).map((m) => {
            ...
            return (
              ...
              >
                ...
                >
                  {isPlaying ? (
                    <Pause size={10} weight="fill" className="text-background" />
                  ) : (
                    <Play size={10} weight="fill" className="text-background translate-x-px" />
                  )}
                </button>

                <div className="min-w-0 flex-1 leading-tight">
                  <span className="font-display text-[13.5px] font-semibold text-foreground">{m.name}</span>
                  <p className="mt-0.5 text-[12.5px] text-muted-foreground truncate italic">"{m.sampleText}"</p>
                </div>
                ...
                <div className="text-right shrink-0">
                  <div className="font-display text-[13px] font-semibold text-foreground">₹{m.ratePer1k.toFixed(2)}</div>
                  <div className="text-[11px] text-muted-foreground">per 1k chars</div>
                </div>
              </div>
            );
          })}
